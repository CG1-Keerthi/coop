...
export class FinancialReconciliationComponent implements OnInit {
  ...
  getReconcilationRecords() {
    ...
    if (this.selectedMonth == undefined || this.selectedMonth == "") {
      this.selectedMonth = "";
    } else {
      this.selectedMonth = this.getMonthNumberBasedOnMonthName(this.selectedMonth);
    }
    ...
    this.mdMondServiceDS.getFormDataFromMondService("Creditor Self Admin", "FetchReconciliationHeaderList", JSON.stringify({ "reportClientName": this.reportClientName, "reportPeriodMonth": this.selectedMonth, "reportPeriodYear": this.selectedYear }), this.csfrToken).subscribe(
      data => {
